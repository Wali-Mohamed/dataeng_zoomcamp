...
b) **pipeline/assets/ingestion/trips.py** - Python asset to fetch NYC taxi parquet data from TLC endpoint
   ...
   - Add `connection: duckdb-default` in the asset metadata (required for materialization)
   - Dependencies: pandas, pyarrow (no requests or python-dateutil needed; use stdlib for date ranges)
   - Return datetime columns as timezone-naive or as ISO strings to avoid "Timezone database not found" on Windows when ingestr loads into DuckDB
...
c) **pipeline/assets/staging/trips.sql** - SQL asset to clean, deduplicate (ROW_NUMBER), and enrich with payment lookup
   ...
   - Custom checks must not query the asset’s own table (e.g. use `ingestion.trips` with the time filter, not `staging.trips`)
...
- **First run** (creates tables): `bruin run ./pipeline/pipeline.yml --full-refresh --start-date 2022-01-01 --end-date 2022-02-01`
- **Windows**: If you see ingestr "Access is denied" or DuckDB "file in use", use a run script that sets `UV_TOOL_DIR` to a project folder and runs with `--workers 1` (single worker so only one asset uses DuckDB at a time). Example: `./run-pipeline.sh --start-date 2022-01-01 --end-date 2022-02-01 --full-refresh`
...
- Verify all quality checks passed (e.g. 6+ checks: column not_null/unique and custom checks)
```
