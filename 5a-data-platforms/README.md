...
> **Windows – ingestr / uv permission errors**: If you see `failed to remove file ... ingestr.exe: Access is denied` or `The process cannot access the file`, uv is trying to update ingestr in `%APPDATA%\uv` and hitting a lock. Use the project run script so tools install in a local folder and DuckDB is used by only one asset at a time:
> ```bash
> ./run-pipeline.sh --start-date 2022-01-01 --end-date 2022-02-01
> ```
> The script sets `UV_TOOL_DIR` and runs with `--workers 1` (required when using a single DuckDB file). On **first run**, use `--full-refresh` so staging and report tables are created:
> ```bash
> ./run-pipeline.sh --start-date 2022-01-01 --end-date 2022-02-01 --full-refresh
> ```
